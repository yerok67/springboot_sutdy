	...
	<table>
		...
		<c:forEach items="${cart.itemSetList}" var="set" varStatus="stat">
			<tr>
				...
				<td><fmt:formatNumber value="${set.quantity * set.item.price}" pattern="###,###" /></td>
				<td><a href="cartDelete?index=${stat.index}">ⓧ</a></td>
			</tr>
		</c:forEach>
		<tr>
			<td colspan="4" align="right">총 구입 금액 : <fmt:formatNumber value="${cart.total}" pattern="###,###" /></td>
		</tr>
	</table>
	<br>${message}<br>
